...
print('SOM Bkq parameters and LF Splitting evaluation initialized. Programmed by Lucca Blois@DQF-IQUSP, Brazil')
...
print('Type the name of your .XYZ molecular geometry file with the extention. It should contain ONLY the coordination polyhedron and be formatted as stanrdard .xyz file.')
print('The Ln3+ ion should be the first atom. Check the example.xyz file if any doubts remain')
File = input()
Input = np.loadtxt(f'{File}',skiprows=2,usecols=(1,2,3))
...
#JOYSpectra g-factors around the 8 oxygen atoms. Best-fit, not necessarily real charges.

g = []

def Charges():
    print('Type the charges of each atoms on the 1st coordination sphere. Example: 1 0.5 0.2 for 3 atoms.')
    print(f'Number of atoms: {len(G)}')
    Charges = input('Charges: ')
    for i in Charges.split():
        g.append(float(i))
    print(f'Are these charges correct y/n? {g}')
    response = input()
    if response == 'y':
        None
    if response == 'n':
        Charges2 = input('Type the charges of the atoms on the 1st coordination sphere:')

Charges()
print(f'Charges to be used in the calculations: {g}')
...
print('Transformation to polar coordinates (physicists scheme): OK')
...
print('Overlap Integrals Calculations: OK')
...
print('Bkq Parameters Calculations: OK')
...
print(Parm)
print('Crystal Field Scalar for J<=1 Nv =',Nv_2)
print('Crystal Field Scalar for J<=2 Nv =',Nv_4)
print('Crystal Field Scalar for J>=3 Nv =',Nv_6)
...
input('All calculations done! Your output file is ready! Press any key to exit')
